...
(function (global) {
  ...
  function rewriteDefaultHeartVoiceMentions(text) {
    ...
    src = src.replace(
      /每轮末尾必须(?:按照要求完整)?输出\s*<miyavoice>\s*心声段（好感度、欲望值、(?:当前状态|行为动作)、角色心声四行）[^。\n]*[。]?/g,
      replacement + '。'
    );
    src = src.replace(
    ...
    src = src.replace(
      /「角色心声」行须写足\s*40\s*字以上（指内心独白，非(?:当前状态|行为动作)）/g,
      '各字段内容须符合「线上格式规则·心声」中的输出要求'
    );
    src = src.replace(
      /好感度、欲望值、(?:当前状态|行为动作)、角色心声四行/g,
      '「线上格式规则·心声」中的全部字段'
    );
    src = src.replace(
      /好感度[、,]\s*欲望值[、,]\s*(?:当前状态|行为动作)[、,]\s*角色心声/g,
      '当前心声规则中的各字段'
    );
    src = src.replace(
      /<miyavoice>\s*(?:当前状态|行为动作)也不得与近几轮雷同/g,
      '<miyavoice> 各字段也不得与近几轮雷同'
    );
    src = src.replace(/心声四行不得出现在正文气泡里/g, '心声字段行不得出现在正文气泡里');
    /* 自定义启用时：删掉仍残留的默认四行字段示例行 */
    src = src.replace(/^\s*好感度-数值[^\n]*$/gm, '');
    src = src.replace(/^\s*欲望值-数值[^\n]*$/gm, '');
    src = src.replace(/^\s*当前状态-[^\n]*$/gm, '');
  ...
  function buildCustomHeartVoicePriorityBlock(roleName, preset) {
    ...
    var lines = [
      ...
      '默认四行（好感度/欲望值/当前状态/角色心声）对本聊天无效，禁止输出。',
