(function (global) {
    ...
    function buildPerTurnFormatReminder(opts) {
        ...
        var lines = [
            '【本轮格式·' + roleName + '】',
            '顺序：<thinking>…</thinking> → 正文（每行一条气泡）；<miyavoice> 是否输出及字段要求以「线上格式规则·心声」为准。',
            '用户只能看到 </thinking> 之后、可选 <miyavoice> 之前的正文；思维链/心声/标记不得泄漏到正文。',
    ...
    /* 行为动作 = 改名前的旧标签，保留以拦截旧聊天/旧预设仍可能泄漏的字段行 */
    var RE_RT_HEART_VOICE =
        /^(?:好感度|欲望值|当前状态|行为动作|角色心声)\s*[-－—：:]/;
    ...
    function buildListenTogetherFormatRules(opts) {
        ...
        return [
            ...
            '',
            '【禁止·思维链与心声】',
            '禁止输出 <thinking>、<miyavoice>、好感度-/欲望值-/当前状态-/角色心声- 等；本轮回复只能是正文气泡。',
            '',
            '【输出要求】',
    ...
    function buildHeartVoiceRulesBlock(roleName, preset, nativeMode) {
        ...
        return [
            '【线上格式规则·心声·' + rn + '】',
            '【可选·完整 miyavoice】本轮可以省略整个 <miyavoice> 心声段；一旦输出，开闭标签均必填，段内四行字段须全部写满，禁止少填、空填、截断或写到一半就结束。',
            '若本轮输出心声，须将其作为正文后的尾段，单独包裹在 <miyavoice>...</miyavoice> 内；禁止写入 <thinking> 或正文气泡行。',
            '若本轮输出心声，须严格按以下四行输出（每行一项，换行分隔；行首标签原样保留，用半角或全角短横线/冒号连接内容）：',
            '好感度-数值（0–100 整数，' + rn + '对用户的当前好感度，输出时须据对话合理更新）',
            '欲望值-数值（0–100 整数，' + rn + '对用户的当前欲望值，输出时须据对话合理更新）',
            ...
            '若本轮输出心声，数值应相对上一轮有合理变化；禁止照搬相同数字、相同状态描写或相同内心独白。',
            '发出前自检：若已开始输出 <miyavoice>，是否已写满好感度/欲望值/当前状态/角色心声四行并正确闭合 </miyavoice>；不足则补全后再结束，否则省略整个心声段。',
            '若系统另行注入「上一轮心声」，本轮如输出心声须对照更新，禁止输出重复/相同的心声（含四行均不得与前一轮雷同）。'
        ].join('\n');
    }
    ...
    function formatHeartVoiceSnapshotLines(entry) {
        ...
        if (String(entry.action || '').trim()) lines.push('当前状态-' + String(entry.action).trim());
    ...
    function buildLastHeartVoiceInjectBlock(chat, contact) {
        ...
        var updateHint = isCustom
            ...
            : '本轮如输出心声，须在此基础上合理更新；禁止输出与上一轮相同或高度重复的心声（好感度、欲望值、当前状态、角色心声均不得照搬或只改一两个词）。';
        return [
            '【上一轮心声·' + roleName + '】',
    ...
    function buildOnlineRules(opts) {
        ...
        var rows = [
            '【线上格式规则】',
            '顺序：<thinking>…</thinking> → 正文（每行一条气泡）；<miyavoice> 是否输出及字段要求以本提示中的「线上格式规则·心声」为准。',
            '思维链只写在 <thinking> 标签内；禁止在正文输出 [正文]、【正文】、[/thinking]、<thinking> 等结构标记。',
            '用户界面只显示 </thinking> 之后、可选 <miyavoice> 之前的正文；正文只输出一遍，禁止先写草稿再原样复读第二轮。',
        ...
        rows = rows.concat([
            ...
            hvPreset
                ? '<miyavoice>' +
                  ...
                  ' 行心声</miyavoice>'
                : '（可选）<miyavoice>四行心声</miyavoice>',
            '',
            '【格式示例·' + roleName + '】',
            hvPreset ? '' : '（以下示例包含可选心声；本轮也可以省略整个 <miyavoice> 段）',
            '<thinking>',
            '（这里写思维过程）',
            '</thinking>'
